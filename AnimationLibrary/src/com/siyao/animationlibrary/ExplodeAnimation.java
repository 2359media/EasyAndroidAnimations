...
public class ExplodeAnimation extends Animation {
	...
	public void animate(final View view) {
		...
		int totalParts = xParts * yParts, bmpWidth = viewBmp.getWidth() / xParts, bmpHeight = viewBmp.getHeight() / yParts, widthCount = 0, heightCount = 0,
				middleXPart = (xParts - 1) / 2;
		...
		for (int i = 0; i < totalParts; i++) {
			...
			imageViews[i].animate().translationXBy(translateX).translationYBy(translateY).alpha(0).setDuration(duration);
	...
	private int[] sideTranslation(int heightCount, int bmpWidth, int bmpHeight, int xParts, int yParts) {
		int[] translation = new int[2];
